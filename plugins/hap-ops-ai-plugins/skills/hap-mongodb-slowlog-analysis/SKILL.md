...
6. 给出结构化建议，并明确说明应当：
   - 先改查询条件
   - 或者可以直接尝试加索引
7. 在提出任何可执行索引命令前，先检查证据是否足够：
   - 是否知道数据库、集合、过滤字段、排序字段和操作类型
   - 是否知道当前 `planSummary`、`keysExamined`、`docsExamined`、`nreturned`、`durationMillis` 中至少一部分
   - 是否需要先查看现有索引，避免重复索引或索引交叉覆盖
   - 如果证据不足，输出“候选索引”或“暂不建议直接建索引”，不要伪造确定结论
8. 把建议分成“诊断结论”和“验证动作”：先说明推断依据，再给出低风险验证命令和成功判据。
...
- 对 `ws*` 集合，已有单字段 `_id` 索引时，默认不把 `_id` 纳入候选复合索引；先用业务等值/范围字段验证过滤收益。
- 只有当查询已改写成正向等值/范围条件，且 `explain("executionStats")` 明确显示排序成本仍是主要瓶颈、把 `_id` 放入复合索引能同时满足排序时，才评估把 `_id` 作为复合索引的排序键，例如 `{ "业务过滤字段": 1, "_id": -1 }`。这属于二次候选，不是默认推荐。
...
- 不要仅凭 `COLLSCAN` 就直接推荐索引；先判断过滤条件的选择性、查询频率、集合规模和是否存在更优的查询改写。
- 不要把 `keysExamined` 高等同于“索引无效”：结合 `docsExamined`、`nreturned`、排序需求和 `limit` 判断索引是否只是部分有效。
- 不要默认把所有过滤字段都放进复合索引；低选择性、负向谓词、数组多键字段和互斥 `$or` 分支要分别评估。
- 索引建议应标注证据等级：`高`（有完整执行统计）、`中`（有慢日志指标但无 explain）、`低`（只有命令体或不完整日志）。

## Validation and Safety

索引建议必须附带可验证的下一步，不把 `createIndex` 当作无风险修复：

- 先用 `db.getCollection("集合名").getIndexes()` 查看现有索引；若无法确认数据库或集合，先要求补充，不要猜测 `use` 目标。
- 优先用脱敏后的原查询执行 `explain("executionStats")`，对比 `executionTimeMillis`、`totalKeysExamined`、`totalDocsExamined`、`nReturned` 和是否出现 `COLLSCAN` / 内存排序。
- 测试候选索引时，先在测试环境或低峰期验证；必要时使用 `hint()` 做对照，但不要把 `hint()` 永久写进业务代码，除非用户明确要求。
- 成功判据至少应包括：扫描量下降、返回结果不变、排序成本可接受、慢查询耗时下降；不能只看“出现了 `IXSCAN`”。
- 同时提醒索引代价：写入放大、内存占用、后台构建时长，以及与已有索引的冗余风险。
- 如果建议改写写入默认值或归一化历史数据，先说明备份、灰度和回滚要求；不要直接给破坏性更新命令，除非用户明确要求并提供字段类型与范围。

## Privacy and Evidence Boundaries

- 日志可能包含业务数据、手机号、邮箱、令牌或内部路径；输出报告时默认脱敏具体值，只保留字段结构、类型和必要的统计信息。
- 严格区分“日志明确显示”“根据日志推断”“需要 explain/索引清单确认”；不要把推断写成事实。
- 缺少 `ns`、数据库名、集合名或完整命令时，允许给出结构性分析，但不得生成看似可直接执行的 `use` 或 `createIndex` 命令。
- 不要为了填满报告而虚构 `sort`、`projection`、索引名称、执行计划或集合大小。
